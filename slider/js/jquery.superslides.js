	...
	((n = function (n, e) {
		...
		var r = {
			...
			h = {
				slide: function (i, t) {
					...
					n
						.eq(i.upcoming_slide)
						.css({ left: i.upcoming_position, display: "block" }),
						s.$control.animate(
							{ left: i.offset },
		...
		var c = {
			...
			d = {
				...
				_addItem: function (i) {
					...
					e && (n = e),
						t("<a>", { href: "#" + n, text: n }).appendTo(s.$pagination);
				},
				_setup: function () {
					if (s.options.pagination && 1 !== s.size()) {
						...
						for (var n = 0; n < s.size(); n++) s.pagination._addItem(n);
					}
				},
				_events: function () {
		...
		return (
			...
			t(i).on("hashchange", function () {
				var i,
					t = s._parseHash();
				(i = t && !isNaN(t) ? s._upcomingSlide(t - 1) : s._upcomingSlide(t)) >=
					0 &&
					i !== s.current &&
					s.animate(i);
			}),
		...
		_prevInDom: function () {
			var i = this.current - 1;
			return i < 0 && (i = this.size() - 1), i;
		},
		_parseHash: function (t) {
			return (
				(t = (t = t || i.location.hash).replace(/^#/, "")) &&
					!isNaN(+t) &&
					(t = +t),
				t
			);
			...
			return (
				this.each(function () {
					...
					if (
						...
						"string" == typeof i && "function" == typeof (o = r[i]))
					)
						return (o = o.call(r, s));
				}),
				o
